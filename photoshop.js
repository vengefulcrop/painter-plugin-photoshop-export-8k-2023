...
ExportConfig.prototype = {
  clone: function() {
    ...
    if (this.hasOwnProperty('resolution')) {
      conf.resolution = this.resolution;
    }
    return conf
  },
...
function PhotoshopExporter(callback) {
  ...
  var resolutionOverride = alg.settings.value("resolutionOverride", [8192, 8192]); // Default to 8k as in QML
  if (resolutionOverride && resolutionOverride.length === 2 && resolutionOverride[0] !== -1) {
    this.exportConfig.resolution = [parseInt(resolutionOverride[0]), parseInt(resolutionOverride[1])];
  }
